...
function validate(section){let ok=true;const require=(name,msg)=>{const el=form.elements[name]||$(`#${name}`);clearError(name);if(!el?.value?.trim()){error(name,msg);ok=false;}};
 if(section==='tipo'&&!form.tipo.value){error('tipo','Selecciona parcela, campo o casa.');ok=false;}
 if(section==='datos'){['region','comuna','localidad'].forEach(n=>require(n,'Completa este dato.'));if(state.type==='casa')require('casaSuperficie','Indica la superficie construida.');else require('superficie','Indica la superficie del terreno.');}
 if(section==='fotos'){
...
function propertyLabel(){if(state.type==='casa')return val('tipoCasa')||'Casa';const area=number(val('superficie'));return val('tipoTerreno')||(area>=10000?'Campo':'Parcela');}
