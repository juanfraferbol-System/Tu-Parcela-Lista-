...
export function calculateValuation(subject={},records=[],parameters={},now=new Date()){
 ...
 const tourismPct=tourism==='nacional'?Number(parameters.ajuste_turismo_nacional??3):tourism==='local'?Number(parameters.ajuste_turismo_local??0.20):0;
 const riverPct=Boolean(subject.acceso_rio??subject.accesoRio)?Number(parameters.ajuste_acceso_rio??0.10):0;
 const applyCommercialPremiums=value=>round(value*(1+tourismPct)*(1+riverPct));
 const minValue=applyCommercialPremiums(applyRouteAdjustment(q20*surface)),quickValue=applyCommercialPremiums(applyRouteAdjustment(q25*surface)),marketValue=applyCommercialPremiums(applyRouteAdjustment(median*surface)),maxValue=applyCommercialPremiums(applyRouteAdjustment(q80*surface));
