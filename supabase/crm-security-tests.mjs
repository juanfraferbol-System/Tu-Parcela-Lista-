...
const crm=await read('../plataforma/crm/index.html');
const crmJs=await read('../plataforma/crm/crm.js');
const crmConfig=await read('../plataforma/crm/crm-config.js');
const admin=await read('../admin-publicaciones.html');
const service=await read('../js/crm-service.js');
...
assert.doesNotMatch(crm+crmJs+crmConfig,new RegExp('const\\s+(?:USER|PASS)|tplCRM'+'Auth'));
assert.match(crm,/id="login-form"/);
assert.match(crmJs,/signInWithPassword/);
assert.match(crmJs,/crm_sesion_actual/);
assert.match(crmJs,/tipo === 'administrador'/);
assert.match(admin,/location\.replace\('CRM\.html'\)/);
...
assert.match(crmJs,/confirm\("¿Estás seguro de APROBAR esta parcela/);
assert.match(crmJs,/crm_moderar_publicacion/);
assert.doesNotMatch(crm+crmJs+crmConfig+service,/service[_-]?role|SUPABASE_SECRET|postgres(?:ql)? password/i);
console.log('OK: CRM oficial, Auth/RPC, perfiles admin activos, estados, auditoría y cero secretos de servidor.');
